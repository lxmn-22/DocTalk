...
const DoctorAppointments = () => {
  ...
  // De-structuring age, appointment-time functions.
            ...
            appointments.reverse().map((item, index)=> (
              <div className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr_1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-100' key={index}>
                ...
                <div className='flex items-center gap-2'>
                  <img className='w-8 rounded-full' src={item.userData.image} alt="userdata-image" /> <p>{item.userData.name}</p>
                </div>
                <div>
                ...
                <p>{currency} {item.amount}</p>
                {
                  ...
                  : item.isCompleted 
                    ...
                    : <div className='flex'>
                      ...
                      <img onClick={()=> completeAppointment(item._id)} className='w-10 cursor-pointer' src={assets.tick_icon} alt="accept-icon" />
                  </div>
                }
              </div>
            ))
